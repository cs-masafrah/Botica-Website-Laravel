@props(['count' => 0, 'navigationLink' => false])

<div class="container mt-14 max-lg:px-8 max-md:mt-7 max-md:!px-0 max-sm:mt-5">
    @if($navigationLink)
    <div class="flex items-center justify-between mb-6 max-md:mb-4 max-sm:mb-3">
        <div class="w-48 h-8 rounded-lg shimmer"></div>
        <div class="w-24 h-6 rounded-lg shimmer"></div>
    </div>
    @endif

    <div class="relative">
        <div class="flex gap-10 overflow-auto scrollbar-hide max-lg:gap-4">
            @for ($i = 0; $i < $count; $i++) <div class="grid min-w-[120px] grid-cols-1 justify-items-center gap-4 max-md:min-w-20 max-md:gap-2.5 max-md:first:ml-4 max-sm:min-w-[60px] max-sm:max-w-[60px] max-sm:gap-1.5">
